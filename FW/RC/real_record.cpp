...
bool TypeRealRecord::GetState( TypeStateReader& state )
{
    //
...
bool TypeRealRecord::SetState( TypeStateWriter& state, TypeStruct* )
{
    //
...
QWidget* TypeRealRecord::PropertyWidget( TypeController& controller )
{
    ...
    //
    // This property selects Realean value:
...
void TypeRealRecord::Html( TypeHtmlBlockStream& block_stream )
{
    //
...
void TypeRealRecord::Script( TypeHtmlBlockStream& block_stream )
{
    //
    // Generate a script:
...
{
    return new TypeRealRecord( TypeRecordFactory::GenerateId(), name, value, parent, root );
}

TypeRecord* TypeRealRecordFactory::NewInstance( TypeStateWriter& state, TypeVariant* parent, TypeStruct* root )
{
    return new TypeRealRecord( state, parent, root );
}
