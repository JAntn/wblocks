...
#include "ui_recordcontainerview.h"
...
C_UiRecordStructView::C_UiRecordStructView(C_Document& document, QWidget *parent) :
    QWidget(parent),
    m_Document(&document),
    ui(new Ui::C_UiRecordContainerView)
{
    ui->setupUi(this);
...
{
    // Get current context record container
...
void C_UiRecordStructView::on_UpButton_clicked()
{
    ...
    {
        C_Record* record = static_cast<C_Record*>(
                    Document().Context().Records().Parent()
                    );

        C_RecordStruct* parent = static_cast<C_RecordStruct*>(
                    record->Parent()
                    );
